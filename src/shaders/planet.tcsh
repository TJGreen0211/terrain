...
in vec3 tNormal[];
in vec3 tTangent[];
in mat4 tInstanceMatrix[];
out vec3 tcPosition[];
out vec3 tcNormal[];
out vec3 tcTangent[];
out mat4 tcInstanceMatrix[];
...
float level(vec3 v0, vec3 v1) {
	...
	return clamp(512.0/length(v0 - v1), 2.0, 16.0);
}
...
void main()
{
	...
	tcInstanceMatrix[ID] = tInstanceMatrix[ID];
	if(ID == 0) {
		gl_TessLevelInner[0] = 1;//l;
	    gl_TessLevelOuter[0] = 1;//l-1.0;
	    gl_TessLevelOuter[1] = 1;//l-1.0;
	    gl_TessLevelOuter[2] = 1;//l-1.0;
	}
}
